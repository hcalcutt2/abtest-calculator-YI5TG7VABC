...
const EXPLAINERS = {
  ...
  aovrpv: {
    ...
    body: "AOV only counts people who purchased. A variant that persuades extra people to make small purchases pushes conversion and revenue per visitor up while pulling average order value down. RPV reflects the full effect on every visitor, which is why it's the headline revenue metric here.",
  },
  mdeabs: {
        ...
        {analysis && (
          <ExportButtons
            ...
            onPdf={() => exportPdf("Revenue analysis", [
              ...
              { heading: "Revenue per visitor (headline)", lines: analysis.rpv.map(r =>
              ...
              { heading: "Average order value (secondary)", lines: analysis.aov.map(r =>
                `${r.name} vs Variant A: ${fmtSignedPct(r.relUplift)}, p=${fmtP(r.pAdj)} — ${(r.pAdj < 1 - confidence) ? (r.relUplift > 0 ? "Significant winner" : (twoTailed ? "Significant loser" : "Not significant")) : "Not significant"}`) },
            ])}
          />
        )}
        ...
        {analysis && (
          <>
            ...
            <div className={analysis.srm && analysis.srm.flagged ? 'dimmed' : ''}>
              <h3 className="sub-title">Revenue per visitor <span className="pill">Headline metric</span></h3>
              {analysis.rpv.map(r => (
              ...
              <h3 className="sub-title">Average order value <span className="pill pill-soft">Secondary</span></h3>
              <p className="field-hint">
                Average order value is the mean order value among buyers only and can move in the opposite direction to revenue per visitor.
              </p>
              {analysis.aov.map(r => (
